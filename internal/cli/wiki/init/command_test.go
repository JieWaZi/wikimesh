...
func TestRunWikiInitInstallsSkillsFromResolvedGitHubBundle(t *testing.T) {
	...
	skillapp.ResolveWikimeshSkills = func(wikiType string, ref string) (skillapp.Bundle, error) {
		...
		return skillapp.Bundle{
			Source: skillapp.NewSource(wikiType, ""),
	...
	cfg, err := wikiapp.LoadRepoConfig("sample")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	if cfg.ProjectName != "Sample" || cfg.ProjectSlug != "sample" || cfg.ActiveSource != wikiapp.SourceLocal {
...
func TestRunWikiInitKeepsResolvedSkillBundleUntilInstallCompletes(t *testing.T) {
	...
	skillapp.ResolveWikimeshSkills = func(wikiType string, ref string) (skillapp.Bundle, error) {
		...
		return skillapp.Bundle{
			...
			Skills: []skillapp.Skill{{Name: "devwiki-query", Description: "query", Dir: sourceDir}},
			Cleanup: func() error {
				cleaned = true
...
func TestRunWikiInitCreateAllowsNoCodeRepos(t *testing.T) {
	...
	cfg, err := wikiapp.LoadRepoConfig("no-code-repo")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	...
	if cfg.ActiveSource != wikiapp.SourceLocal || cfg.Sources.Local == nil || !samePath(t, cfg.Sources.Local.Path, targetDir) {
...
func TestRunWikiInitCreateSavesExplicitCodeRepos(t *testing.T) {
	...
	cfg, err := wikiapp.LoadRepoConfig("create-linked-code")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
...
func TestRunWikiInitCreateUsesProjectSlugDirectory(t *testing.T) {
	...
	cfg, err := wikiapp.LoadRepoConfig("my-project")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
...
func TestRunWikiInitLinkLocalWikiRepo(t *testing.T) {
	...
	err := runWikiInit(context.Background(), io.Discard, false, InitOptions{
		...
		SourceType:  wikiapp.SourceLocal,
		LocalPath:   wikiRoot,
	...
	cfg, err := wikiapp.LoadRepoConfig("linked-local")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	if cfg.ActiveSource != wikiapp.SourceLocal || cfg.Sources.Local == nil || !samePath(t, cfg.Sources.Local.Path, wikiRoot) {
...
func TestRunWikiInitLinkUsesExplicitCodeRepoSlug(t *testing.T) {
	...
	cfg, err := wikiapp.LoadRepoConfig("linked-explicit")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	if len(cfg.CodeRepos) != 1 {
...
func TestRunWikiInitLinkRemoteWikiRepo(t *testing.T) {
	...
	err := runWikiInit(context.Background(), io.Discard, false, InitOptions{
		...
		SourceType:  wikiapp.SourceRemote,
	...
	cfg, err := wikiapp.LoadRepoConfig("linked-remote")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	if cfg.ActiveSource != wikiapp.SourceRemote || cfg.Sources.Remote == nil || cfg.Sources.Remote.URL != "https://example.test/wikimesh" {
...
func TestCollectInitOptionsReadsLinkModeFromNonInteractiveInput(t *testing.T) {
	...
	if opts.ProjectName != "Linked Remote" || opts.SourceType != wikiapp.SourceRemote || opts.RemoteURL != "https://example.test/wikimesh" {
...
func TestNewCommandSupportsNonInteractiveRemoteLinkMode(t *testing.T) {
	...
	cfg, err := wikiapp.LoadRepoConfig("remote-cli")
	if err != nil {
		t.Fatalf("LoadWikiRepoConfig error = %v", err)
	}
	if cfg.ActiveSource != wikiapp.SourceRemote || cfg.Sources.Remote == nil || cfg.Sources.Remote.URL != "https://example.test/wiki" {
		t.Fatalf("remote source = %#v active=%q", cfg.Sources.Remote, cfg.ActiveSource)
	}
}
...
func TestWikiInitValidationMessagesAreChinese(t *testing.T) {
	tests := []struct {
		...
		{name: "missing local path", opts: InitOptions{Mode: InitModeLink, ProjectName: "Sample", SourceType: wikiapp.SourceLocal}},
		{name: "missing remote url", opts: InitOptions{Mode: InitModeLink, ProjectName: "Sample", SourceType: wikiapp.SourceRemote}},
	}
...
func TestSelectedWikiInitSkillsPromptsTypeNextToSkillSelection(t *testing.T) {
	...
	t.Cleanup(func() { skillapp.ResolveWikimeshSkills = originalResolver })
	skillapp.ResolveWikimeshSkills = func(wikiType string, ref string) (skillapp.Bundle, error) {
		if wikiType != skillapp.DefaultType() {
	...
	wikiSelectOne = func(options ui.SelectOneOptions) (string, bool, error) {
		...
		return skillapp.DefaultType(), false, nil
	}
...
func stubWikiSkillResolverWithSkill(t *testing.T, name string) {
	...
	skillapp.ResolveWikimeshSkills = func(wikiType string, ref string) (skillapp.Bundle, error) {
		return skillapp.Bundle{
			Source: skillapp.NewSource(wikiType, ref),
			Skills: []skillapp.Skill{{Name: name, Description: "test", Dir: sourceDir}},
		}, nil
	}
}
