...
func runWikiSearch(ctx context.Context, out io.Writer, root, project, kind string, queries []string) error {
	...
	case "topic", "workflow":
		if ok, err := wikiapp.SearchPagesWithQMD(ctx, out, resolvedRoot, kind, strings.Join(queries, " ")); ok || err != nil {
			return err
		}
		return searchPagesLocal(out, resolvedRoot, kind, query)
	default:
...
func searchPagesLocal(out io.Writer, root, kind, query string) error {
