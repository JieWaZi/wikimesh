...
func runWikiRead(out io.Writer, root, project, kind, slug, view, format string) error {
	...
	page, err := loadPage(resolvedRoot, kind, slug)
	if err != nil {
		return err
	}
	if view == "" {
...
// loadPage 按约定路径加载一个 topic/workflow 页面。
func loadPage(root, kind, slug string) (wikiapp.Page, error) {
	if kind != "topic" && kind != "workflow" {
		return wikiapp.Page{}, fmt.Errorf("unsupported wiki page kind %q", kind)
	}
	rel := filepath.ToSlash(filepath.Join("wiki", kind+"s", slug+".md"))
	return wikiapp.ParsePage(root, rel)
}
