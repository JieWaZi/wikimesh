...
func TestNewWikimeshSkillsSourceDefaultsToGitHub(t *testing.T) {
	t.Setenv("WIKIMESH_SKILLS_REF", "")

	source := NewWikimeshSkillsSource("devwiki", "")
...
func TestNewWikimeshSkillsSourceUsesEnvRef(t *testing.T) {
...
func TestDiscoverWikiSkillsFindsDevwikiSkills(t *testing.T) {
	root := t.TempDir()
	skillDir := root + "/query"
	...
	found, err := DiscoverWikiSkills(root)
	if err != nil {
		t.Fatalf("DiscoverWikiSkills error = %v", err)
	}
	if len(found) != 1 {
